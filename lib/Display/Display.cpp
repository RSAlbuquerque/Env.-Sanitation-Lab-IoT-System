...
Display::Display(uint8_t w, uint8_t h, uint8_t resetPin, uint8_t i2cAddr)
    : _oled(w, h, &Wire, resetPin), _i2cAddr(i2cAddr), _oledReady(false), _currentPage(0), _lastPageSwitch(0) {}
...
void Display::modeSelector(bool useEAP) {
    ...
    auto message = useEAP ? String("Edu[X] Hts[ ] v") + _version : String("Edu[ ] Hts[X] v") + _version;
...
void Display::airSensorValues(const AirValues &data) {
    ...
    if (millis() - _lastPageSwitch > 3000) {
        _currentPage++;
        if (_currentPage > 3)
            _currentPage = 0; // Reset after Page 3
